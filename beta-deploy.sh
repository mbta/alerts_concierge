...
# determine the project and app directories
PROJECT_DIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" && pwd )"
APP_DIR=$PROJECT_DIR/apps/concierge_site
...
cd $APP_DIR
MIX_VERSION_NUMBER=`(mix do clean, compile, release.version show 1>&2;) 2>&1 | grep +`
MIX_VERSION_PREFIX=$(echo $MIX_VERSION_NUMBER | awk -F'+' '{print $1}')
...
docker cp \
$DOCKER_CONTAINER_ID:/alerts_concierge/releases/concierge_site/releases/$MIX_VERSION_PREFIX/concierge_site.tar.gz \
...
# github release tag
git tag -a $MIX_VERSION_NUMBER -m "deployed ${MIX_VERSION_NUMBER} to beta server"
