...
CONTAINER_ID=$(docker run -t -d ${BUILD_TAG} /bin/bash)
...
docker cp \
$CONTAINER_ID:/alerts_concierge/releases/$ALERT_APP/releases/$ALERT_VERSION_NUMBER/$ALERT_APP.tar.gz \
$PREFIX/$ALERT_APP/.deliver/releases/${ALERT_APP}_$ALERT_VERSION_NUMBER.tar.gz

docker cp \
$CONTAINER_ID:/alerts_concierge/releases/$CONCIERGE_APP/releases/$CONCIERGE_VERSION_NUMBER/$CONCIERGE_APP.tar.gz \
