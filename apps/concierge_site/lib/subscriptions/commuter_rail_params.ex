defmodule ConciergeSite.Subscriptions.CommuterRailParams do
  ...
  alias AlertProcessor.{Model.Subscription, Subscription.CommuterRailMapper}
  ...
  def prepare_for_mapper(%{"relevant_days" => relevant_days, "origin" => origin, "destination" => destination, "trips" => trips, "return_trips" => return_trips, "trip_type" => "round_trip"} = params) do
    trip_schedule_map = CommuterRailMapper.trip_schedule_info_map(origin, destination, relevant_days)
  ...
  def prepare_for_update_changeset(subscription, params) do
    with %{"alert_priority_type" => alert_priority_type, "trips" => trips} <- params,
         ...
         trip_schedule_map <- CommuterRailMapper.trip_schedule_info_map(subscription.origin, subscription.destination, relevant_days),
         {start_time, end_time} <- subscription_timestamps(trip_schedule_map, subscription.origin, subscription.destination, trips) do
      {:ok, %{
