defmodule ConciergeSite.Dissemination.MailerInterface do
  ...
  def handle_call({:send_notification_email, notification}, _from, _state) do
    ...
    Logger.info(fn -> "Notification Email result: #{inspect(response)}, alert_id: #{notification.alert_id}, user_id: #{notification.user.id}, notification_id: #{notification.id}" end)
    {:reply, response, nil}
  end
