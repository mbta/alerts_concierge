defmodule ConciergeSite.AlertFactory do
  ...
  defp inclusive_active_period(),
    do: %{"start" => timestamp(), "end" => timestamp() + 100 * 365 * 24 * 60 * 60}

  defp inclusive_severity(), do: 9
  # Monday, January 1, 2018 12:00:00 AM GMT-05:00 (EST)
  def timestamp(), do: 1_514_782_800
  ...
  def active_period(start_time, end_time, :sunday) do
    %{
      "start" => alert_time(start_time) + @one_day * 6,
      "end" => alert_time(end_time) + @one_day * 6
    }
  end

  def active_period(start_time, end_time, :saturday) do
    %{
      "start" => alert_time(start_time) + @one_day * 5,
      "end" => alert_time(end_time) + @one_day * 5
    }
  end
