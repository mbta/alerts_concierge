defmodule AlertProcessor.Dissemination.MassNotifier do
  ...
  defp save_batch(notifications) do
    ...
    Repo.transaction(fn ->
      Repo.insert_all(
        Notification,
        Enum.map(notifications_with_id, &notification_record(&1, now))
      )

      Repo.insert_all(
        NotificationSubscription,
        Enum.flat_map(notifications_with_id, &notification_subscription_records(&1, now))
      )
    end)
  ...
  defp notification_record(%Notification{id: id} = notification, now) when not is_nil(id) do
    ...
    |> Map.merge(%{status: :sent, inserted_at: now, updated_at: now})
  end

  @notification_subscription_fields NotificationSubscription.__schema__(:fields) -- [:id]

  defp notification_subscription_records(
         %Notification{
           id: notification_id,
    ...
    |> Enum.map(fn notification_subscription ->
      ...
      |> Map.merge(%{notification_id: notification_id, inserted_at: now, updated_at: now})
    end)
  end
