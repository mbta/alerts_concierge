defmodule AlertProcessor.Subscription.DisplayInfo do
  ...
  @spec departure_times_for_subscriptions([Subscription.t()], Date.t() | nil) ::
          {:ok, map} | :error
  ...
  defp parse_subscription_stops_and_relevant_days(subscriptions) do
    relevant_subscriptions =
      Enum.filter(subscriptions, &Enum.member?([:commuter_rail, :ferry], &1.type))

    stops =
      relevant_subscriptions
      |> Enum.flat_map(fn subscription ->
        for informed_entity <- subscription.informed_entities,
            InformedEntity.entity_type(informed_entity) == :stop do
  ...
  defp fetch_schedule_info(stops, relevant_days, selected_date) do
    Enum.reduce(relevant_days, {[], []}, fn relevant_day, {previous_schedules, previous_trips} ->
      relevant_date =
        selected_date || DateTimeHelper.determine_date(relevant_day, Date.utc_today())
