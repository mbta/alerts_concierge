defmodule AlertProcessor.Subscription.Mapper do
  ...
  defmacro __using__(_) do
    quote do
      ...
      defp map_type(subscriptions, type) do
        Enum.map(subscriptions, fn(subscription) ->
      ...
      defp map_stops([{subscription, informed_entities}], %{"origin" => origin, "destination" => destination}, routes) do
        stop_entities =
          Enum.flat_map(routes, fn(%Route{route_id: route, route_type: type}) ->
            [
              %InformedEntity{route: route, route_type: type, stop: origin},
      ...
      defp map_trips([{sub1, ie1}, {sub2, ie2}], %{"trips" => trips, "return_trips" => return_trips}) do
        trip_entities =
      ...
      defp map_trips([{subscription, informed_entities}], %{"trips" => trips}) do
        trip_entities =
          Enum.map(trips, fn(trip) ->
            %InformedEntity{trip: trip}
          end)
